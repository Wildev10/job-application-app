...
/**
 * Render the public job page and submit applications for a specific company/job pair.
 */
export default function ApplyByJobPage() {
  const params = useParams();
  const companySlug = typeof params?.slug === 'string' ? params.slug : '';
  const jobSlug = typeof params?.jobSlug === 'string' ? params.jobSlug : '';
  ...
  useEffect(() => {
    const loadJob = async () => {
      ...
      try {
        const payload = await apiFetch(`/jobs/public/${companySlug}/${jobSlug}`, { method: 'GET' });
        setJob(payload);
      } catch {
        setNotFound(true);
      } finally {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      await apiFetch(`/applications/${companySlug}/${jobSlug}`, {
        method: 'POST',
        body: payload,
      });
  ...
  if (loading) {
    return (
      <main className="min-h-[calc(100vh-3.5rem)] bg-[#fafaf9] px-4 py-10 sm:px-6">
        <section className="mx-auto w-full max-w-3xl rounded-2xl border border-[#e5e7eb] bg-white p-6 sm:p-8">
          <div className="h-8 w-52 animate-pulse rounded bg-[#eef0f3]" />
          <div className="mt-3 h-4 w-72 animate-pulse rounded bg-[#eef0f3]" />
          <div className="mt-8 h-40 w-full animate-pulse rounded bg-[#eef0f3]" />
          <div className="mt-6 h-56 w-full animate-pulse rounded bg-[#eef0f3]" />
        </section>
      </main>
    );
  }
  ...
  if (notFound || !job) {
    return (
      <main className="flex min-h-[calc(100vh-3.5rem)] items-center justify-center bg-[#fafaf9] px-4 py-8">
        <section className="w-full max-w-lg rounded-xl border border-amber-200 bg-white p-8 text-center">
          <h1 className="text-2xl font-extrabold text-[#0f0f0f]">Poste indisponible</h1>
          <p className="mt-3 text-sm text-[#737373]">
            Ce poste n&apos;est plus disponible ou n&apos;existe pas.
          </p>
        </section>
      </main>
    );
  ...
  return (
    <main className="min-h-[calc(100vh-3.5rem)] bg-[#fafaf9] px-4 py-10 sm:px-6">
      <section className="mx-auto w-full max-w-3xl rounded-2xl border border-[#e5e7eb] bg-white p-6 shadow-sm sm:p-8">
        <header className="mb-8 border-b border-[#ececec] pb-5">
          <div className="flex items-center gap-3">
            {job.company?.logo ? (
              ...
              <div className="flex h-10 w-10 items-center justify-center rounded-md bg-[#f3f4f6] text-sm font-semibold text-[#525252]">
                {job.company?.name?.slice(0, 1)?.toUpperCase() || 'C'}
              </div>
            )}
            <div>
          ...
          {job.description && (
            <p className="mt-4 whitespace-pre-line text-sm leading-6 text-[#4b5563]">{job.description}</p>
          )}

          {job.expires_at && (
            <p className="mt-4 text-sm font-medium text-orange-600">
              Date limite : {new Intl.DateTimeFormat('fr-FR', { dateStyle: 'long' }).format(new Date(job.expires_at))}
            </p>
          )}
        </header>
